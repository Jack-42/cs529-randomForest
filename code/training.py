...
def main():
    ...
    feat_rs = np.arange(0.1, 0.4, 0.1)
    metrics = {"entropy": entropy, "gini": gini,
               "misclassification_error": misclassification_error}
    alphas = [0.05, 0.01]
    bag_rs = np.arange(0.1, 0.5, 0.1)
    ...
    pbar = tqdm(
        total=len(split_seeds) * len(seeds_for_bag_seed_generator) * len(
            seeds_for_feat_bag_seed_generator) * len(tree_counts) * len(
            bag_rs) * len(feat_rs) * len(alphas) * len(metrics))
                        ...
                        for feat_r in feat_rs:
                            for alpha in alphas:
                                for metric in metrics:
                                    forest = RandomForest(feat_r,
                                                          metrics[metric],
                                                          alpha, bag_r,
                                                          tree_count, bag_seed,
                                                          feat_bag_seed)
                                    forest.train(train_with)
                                    classifications = forest.classify(
                                        val_df_no_class)
                                    acc = get_accuracy(classifications["class"],
                                                       val_df_set_index[
                                                           "class"])
                                    avg_depth = forest.avg_tree_depth()
                                    max_depth = forest.max_tree_depth()
                                    top_feat = forest.most_common_top_feature()
                                    res.add_entry([feat_r, metric, alpha, bag_r,
                                                   tree_count, split_seed,
                                                   max_depth, avg_depth, acc,
                                                   top_feat, bag_seed,
                                                   feat_bag_seed])
                                    pbar.update(1)
    ...
    res.results_df.to_csv("../data/grid_searchmike_tiny.csv")
