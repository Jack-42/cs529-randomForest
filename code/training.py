...
def main():
    ...
    df = pd.read_csv(pth)
    split_seeds = np.arange(1, 11, 1)
    feat_rs = np.arange(0.1, 0.8, 0.1)
    metrics = {"entropy": entropy, "gini": gini, "misclassification_error": misclassification_error}
    alphas = [0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.99]
    bag_rs = np.arange(0.1, 0.8, 0.1)
    tree_counts = np.arange(10, 110, 10)
    seeds_for_bag_seed_generator = np.arange(1, 11, 1)
    seeds_for_feat_bag_seed_generator = np.arange(1, 11, 1)
    ...
    res.results_df.to_csv("../data/grid_search1.csv")
